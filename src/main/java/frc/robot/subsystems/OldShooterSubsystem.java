...
public class OldShooterSubsystem extends SubsystemBase {
  ...
  public void steadyShoot(double velocity) {
    ...
    final double shootFeedforward = calculateNew(velocity, 0, 0.19, 2.4, 0); 
    ...
    double shootOutput = shooterPID.execute(velocity, getAverageEncoderVelocityMPS());
    
    // final voltage command going to falcon or talon (percent voltage, max 12 V)
    double shoot_speed_cmd = ((shootOutput + shootFeedforward) / batteryVoltage);
    ...
    SmartDashboard.putNumber("Shooter Motor Command", shoot_speed_cmd);
    ...
    SmartDashboard.putNumber("Left Shooter Encoder MPS", getLeftEncoderVelocityMetersPerSecond());
    SmartDashboard.putNumber("Right Shooter Encoder MPS", getRightEncoderVelocityMetersPerSecond());
  ...
  public double getVelocityOfWheel() {
    ...
    System.out.println("Velocity of ball: " + initialVelocityMPS);
    ...
    System.out.println("Velocity of wheel: " + velocityOfWheel);

    return (velocityOfWheel / 0.1); // divide by 10 to get 100 MS unit
  }

  public void shootUpperHub() {
    steadyShoot(getVelocityOfWheel());//*1.1);
  }
